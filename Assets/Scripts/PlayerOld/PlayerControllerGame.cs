...
{/*
    ...
    void Start ()
    {
        _normalSprite = mummieSprite.GetComponent<SpriteRenderer>().sprite;
        _hit = false;
        _white = false;
        controller = GetComponent<Controller2D>();
        gravity = -(2 * JumpHeight) / Mathf.Pow(forceFirstJump, 2);
        jumpVelocity = Mathf.Abs(gravity) * forceFirstJump;
        transform.position = spawnData.map2[playerId - 1];
    ...
    public void Reset()
    {
        if (GetComponent<PlayerControllerGame>().enabled == true)
        {
            ...
            mummieSprite.transform.eulerAngles = Vector3.zero;
            _isAttacking = false;
        }
    }
	...
	void Update ()
    {
        ...
        if (health <= 15)
        {
            t.color = colorHealth[2];
        }
        else if (health > 15 && health <= 25)
        {
            t.color = colorHealth[1];
        }
        else
        {
            t.color = colorHealth[0];
        }
    ...
    void Death()
    {
        mummieSprite.GetComponent<SpriteRenderer>().sprite = deathSprite;
        mummieSprite.transform.eulerAngles = new Vector3(0, 0, -90);
        _anim.SetTrigger("ajustAttack");
        _anim.SetBool("prepareAttack", false);
